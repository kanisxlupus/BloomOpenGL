...
void Model::loadMesh(unsigned int indMesh)
{
	...
	vector<GLuint> indicies = getIndices(JSON["accessors"][indAccInd]);
...
vector<unsigned char> Model::getData()
{
	...
	string fileDictionary = fileStr.substr(0, fileStr.find_last_of('/') + 1);
	bytesText = get_file_contents((fileDictionary + uri).c_str());
...
vector<float> Model::getFloats(json accessor)
{
	...
	else throw std::invalid_argument("Model::GetFloats(): Type is invalid (not SCALAR, VEC2, VEC3, or VEC4)");
...
vector<Vertex> Model::assembleVertices(vector<glm::vec3> positions, vector<glm::vec3> normals, vector<glm::vec2> texUVs)
{
